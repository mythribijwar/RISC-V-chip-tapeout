...
**Why do we need SPICE simulations?** SPICE simulations verify circuit behavior and performance before implementation. In **circuit design**, logic gates like NAND, NOR, and NOT are built using NMOS and PMOS transistors.  
...
**NMOS Transistor**: An N-type Metal-Oxide-Semiconductor that conducts when a positive voltage is applied to its gate, enabling current flow from drain to source.
 ![Screenshot 2024-12-12 011430](https://github.com/user-attachments/assets/7124668a-4be3-4dfb-a1ba-b8816e0344ff)
**Threshold Voltage** ($V_t$): A key parameter that drives SPICE simulations, representing the voltage at which a transistor switches from off to on.  
...
**Threshold Voltage at $V_{sb} = 0$**: The threshold voltage when the source-to-body voltage is zero.  

**Body Effect Coefficient ($\gamma$) and Fermi Potential ($\phi_f$)**: Constants provided by the foundry that form the transistor model and are fed into SPICE simulations for various calculations.
...
**$C_{ox}$**: A constant value provided by the foundry, representing oxide capacitance per unit area.
...
SPICE simulations allow us to calculate the **drain current ($I_d$)** for different **$V_{gs}$** values by sweeping **$V_{ds}$** for each **$V_{gs}$** until $V_{ds} = V_{gs} - V_t$ is reached. This helps in generating accurate **$I_d$-$V_{ds}$** curves and understanding the transistor's behavior.

**Saturation Region/Pinch-off Region Condition**:  
In the saturation (or pinch-off) region, the drain current **$I_d$** becomes independent of **$V_{ds}$** and depends primarily on **$V_{gs}$**, as long as **$V_{ds} \ge V_{gs} - V_t$**. This is where the transistor is fully "on" and conducting.
...
**SPICE Conclusion to Saturation Operation**:  
SPICE simulations allow us to calculate the **drain current ($I_d$)** for different **$V_{gs}$** values by sweeping **$V_{ds}$** for each **$V_{gs}$** until $V_{ds} = V_{gs} - V_t$ is reached, and continuing the sweep into the saturation region.
...
### Introduction to Basic Element in Circuit Design - PMOS Transistor

**PMOS Transistor**: A P-type Metal-Oxide-Semiconductor that conducts when a *negative* voltage (or a voltage significantly lower than VDD) is applied to its gate, enabling current flow from source to drain.
* **Source** is typically connected to **VDD**.
* **Drain** is connected to the load or output.
* It is **ON** when $V_{gs}$ is near $0V$ (logic LOW) and **OFF** when $V_{gs}$ is near $V_{DD}$ (logic HIGH).
* In CMOS logic, PMOS transistors are used in the **pull-up network**.

---

### SPICE Netlist Example

A **SPICE Netlist** is a text file that describes a circuit's components, connections, and simulation commands.

-   **M1 vdd n1 0 0 nmos W=1.8u L=1.2u**: Defines an **NMOS** transistor ($M_1$) with **drain** at `vdd`, **gate** at `n1`, **source** at `0` (GND), and **substrate** at `0` (GND).
-   **R1 in n1 55**: Defines a **resistor** ($R_1$) with value $55\Omega$ between node `in` and node `n1`.
-   **Vdd vdd 0 2.5**: Defines a **DC voltage source** ($V_{DD}$) of $2.5V$ between node `vdd` and ground (`0`).
-   **Vin in 0 2.5**: Defines an **input DC voltage source** ($V_{in}$) of $2.5V$ between node `in` and ground (`0`).
...
SPICE (Simulation Program with Integrated Circuit Emphasis) is a powerful tool used for simulating the behavior of electronic circuits. It allows circuit designers to analyze and validate designs before fabrication.

**SPICE Lab with Sky130 Models**

To use SPICE with **Sky130 technology**, you can clone the relevant GitHub repository containing models and circuits:
...
1.  **`/sky130CircuitDesignWorkshop/design/sky130_fd_pr/cells/nfet_01v8/sky130_fd_pr__nfet_01v8__tt.pm3.spice`**: SPICE model for the **NFET (N-channel MOSFET)** at typical (tt) conditions.
2.  **`/sky130CircuitDesignWorkshop/design/sky130_fd_pr/cells/nfet_01v8/sky130_fd_pr__nfet_01v8__tt.corner.spice`**: Provides the **corner model** for the NFET, used for simulating different process variations.
3.  **`/sky130CircuitDesignWorkshop/design/sky130_fd_pr/models/sky130.lib.pm3.spice`**: The main library file containing all the **SPICE models** for components in the Sky130 process node.
...
The following netlist simulates the **$I_d$ vs $V_{ds}$** curves for an NMOS transistor.
...
#### Understanding the `.dc` Simulation Command

The **`.dc`** command performs a **DC sweep analysis**. In the example:
`.dc **Vdd 0 1.8 0.1** **Vin 0 1.8 0.2**`

1.  **First Sweep Variable (Vdd)**: Sweeps $V_{ds}$ from $0V$ to $1.8V$ with a $0.1V$ step.
2.  **Second Sweep Variable (Vin)**: Sweeps $V_{gs}$ from $0V$ to $1.8V$ with a $0.2V$ step.

This setup calculates the drain current ($I_d$) by sweeping $V_{ds}$ for multiple fixed values of $V_{gs}$.
...
![WhatsApp Image 2024-12-10 at 9 31 06 PM](https://github.com/user-attachments/assets/5b9a1024-cd61-42c1-a83d-e4484b8d77e4)
